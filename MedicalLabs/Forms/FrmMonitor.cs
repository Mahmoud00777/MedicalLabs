    ...
    public partial class FrmMonitor : Form
    {
        ...
        private void FrmMonitor_Load(object sender, EventArgs e)
        {
           FunctionsClass.userTrafficRegister(this.Name, true);
            ...
            dgvBuyInvo.DefaultCellStyle.Font = new Font("Times New Roman", 11, FontStyle.Regular);
        ...
        void getData()
        {
            try
            {
                ...
                // حركة الفواتير
                SQLConClass.sqlQuery = "SELECT TblEvents.*,TblUser.UserName FROM TblEvents,TblUser where  TableName='TblInvoices' AND TblUser.Id=TblEvents.UserId AND"
                ...
                if (cmbUserName.SelectedIndex == -1)
                {
                    ...
                    SQLConClass.sqlQuery += " SELECT TblUserTraffices.*,TblUser.userName FROM TblUserTraffices,TblUser " +
                                            "where TblUser.Id=TblUserTraffices.UserId AND DateEntry BETWEEN '" + TrafficDateTimeFrom.ToString() + "'AND '" + TrafficDateTimeTo.ToString() + "' order by Id desc";
                }
                else
                {
                    // حركة المستخدم المحدد من الكومبو خلال التاريخ المحدد
                    SQLConClass.sqlQuery += " SELECT TblUserTraffices.*,TblUser.UserName FROM TblUserTraffices,TblUser " +
                                            "where TblUser.Id=TblUserTraffices.UserId AND DateEntry BETWEEN '" + TrafficDateTimeFrom.ToString() + "'AND '" + TrafficDateTimeTo.ToString() + "' AND TblUsers.Id=" + cmbUserName.SelectedValue + "  order by Id desc";
                }

                // تحميل كل المستخدمين الذين استخدمو المنظومة خلال 90 يوم في شاشة مراقبة الشبكة
                SQLConClass.sqlQuery += " SELECT TblUserTraffices.*,TblUser.UserName FROM TblUserTraffices,TblUser " +
                                        "where TblUser.Id=TblUserTraffices.UserId AND DATEDIFF(DD,DateEntry,GETDATE())<=90 ";

                // تحميل أسماء المستخدمين في الكومبو
                SQLConClass.sqlQuery += " SELECT Id, UserName FROM TblUser WHERE Del='FALSE'";

                // ----------------------------الفواتير  الشراء --------------------------------
                long InvoBuyDateDays;
                InvoBuyDateDays = Convert.ToInt64(dtpBuyInvoFrom.Value.Date.Subtract(dtpBuyInvoTo.Value.Date).Days);

                if (InvoBuyDateDays > 1 | InvoBuyDateDays < 0)
                {
                    timer1.Enabled = false;
                    MessageBox.Show("لا يمكن ان يكون تاريخ العرض اكبر من يوم واحد", "مراقبة فواتير المبيعات", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    dtpBuyInvoFrom.Value = DateTime.Now;
                    dtpBuyInvoTo.Value = DateTime.Now;
                    timer1.Enabled = true;
                    return;

                }

                DateTime InvoDateBuyInvoTimeFrom = Convert.ToDateTime(dtpBuyInvoFrom.Value.Date + dtpBuyInvoDateFrom.Value.TimeOfDay);
                DateTime InvoDateBuyInvoTimeTo = Convert.ToDateTime(dtpBuyInvoTo.Value.Date + dtpBuyInvoDateTo.Value.TimeOfDay);

                // حركة الفواتير
                SQLConClass.sqlQuery += " SELECT TblEvents.*,TblUser.UserName FROM TblEvents,TblUser where  TableName='TblBuyInvoice' AND TblUser.Id=TblEvents.UserId AND"
                    + " EventDate BETWEEN '" + InvoDateBuyInvoTimeFrom + "'AND '" + InvoDateBuyInvoTimeTo.ToString() + "'";
...
        }
        ...
        void fillDGVBuyInvo()
        {
            dgvBuyInvo.Rows.Clear();
            for (int i = 0; i <= dsMonitor.Tables[4].Rows.Count - 1; i++)
            {
                dgvBuyInvo.Rows.Add();

                dgvBuyInvo.Rows[i].Cells[0].Value = i + 1;
                dgvBuyInvo.Rows[i].Cells[1].Value = dsMonitor.Tables[4].Rows[i][1];
                dgvBuyInvo.Rows[i].Cells[2].Value = dsMonitor.Tables[4].Rows[i][2];
                dgvBuyInvo.Rows[i].Cells[3].Value = dsMonitor.Tables[4].Rows[i][7];
                dgvBuyInvo.Rows[i].Cells[4].Value = dsMonitor.Tables[4].Rows[i][6];
            }
            dgvBuyInvo.ClearSelection();
            //if (dgvBuyInvo.RowCount != 0)
            //    dgvBuyInvo.FirstDisplayedScrollingRowIndex = dgvInvoiceMonitor.RowCount - 1;
        }
        ...
        private void btnDelegateShow_Click(object sender, EventArgs e)
        {
            getData();
            fillDGVBuyInvo();
        }
        ...
        {
            dtpBuyInvoFrom.Value = DateTime.Now; DateTime.Now.AddDays(Convert.ToInt32(-1));
            dtpBuyInvoTo.Value = DateTime.Now;
            dtpBuyInvoDateFrom.Value = Convert.ToDateTime(dtpInvoDateFrom.Value.Date.ToShortDateString() + " 12:00:00 AM");
            dtpBuyInvoDateTo.Value = Convert.ToDateTime(dtpInvoDateTo.Value.Date.ToShortDateString() + " 11:59:59 PM");
