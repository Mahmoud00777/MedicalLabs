...
namespace MedicalLabs.Forms
{
    public partial class FrmCustomer : Form
    {
        ...
        public void getData()
        {
            ...
            ConClass.SqlQuery = "SELECT Id, ROW_NUMBER() OVER (ORDER BY (SELECT 1)) AS ت ,[Name] , Company ,CONVERT(NVARCHAR,Phone,100) ,Email,[Address],Id,Del FROM TblResource WHERE Del=0 ";
            ConClass.SqlQuery += " ORDER BY [Name] OFFSET " + pageSize * (pageNum - 1) + " ROWS FETCH NEXT " + pageSize + " ROWS ONLY";
            ConClass.SqlQuery += " SELECT COUNT(Id) FROM TblResource WHERE Del=0";
        ...
        public void btnSaveCustomer_Click(object sender, EventArgs e)
        {
            ...
            // ------------------------------------------------------------------
            DataRow[] perm = VariablesClass.dtUserPermissions.Select("OperationID=14");
            ...
            dsCustomer = sqlCon.cmdExecuteData("SaveResource", param);
            if (FunctionsClass.dsHasTables(dsCustomer))
            {
                fillData();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (DialogResult.Yes == MessageBox.Show("هل أنت متأكد تريد حذف هذا المورد؟", "تأكيد حذف", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
            {
                var sqlCon = new SQLConClass();
                SqlParameter[] param = new SqlParameter[] { new SqlParameter("@Id", customerId) };
                dsCustomer = sqlCon.cmdExecuteData("DeleteResourece", param);
                if (FunctionsClass.dsHasTables(dsCustomer))
                {
                    fillData();
                    FrmInvoice frmInvoice = new FrmInvoice();
