...
namespace MedicalLabs.Forms
{
    public partial class FrmCharts : Form
    {
        ...
        private void GetItem()
        {
            ...
            {
                dTItem.Clear();
                SQLConClass.sqlQuery = "SELECT *  FROM TblAnalysis WHERE DEL='FALSE' ORDER BY AnalysisName ASC";
            ...
            cmbItemChart.DisplayMember = "AnalysisName";
        ...
        private void Chart()
        {
            ...
            dTChart.DefaultView.Sort = "Price DESC";
            dTChart = dTChart.DefaultView.ToTable();

            chrtItemPrice.Series["Series1"].XValueMember = dTChart.Columns["AnalysisName"].ToString();
            chrtItemPrice.Series["Series1"].YValueMembers = dTChart.Columns["Price"].ToString();
...

            {
                ...
                {
                    dTChart = new DataTable();
                    SQLConClass.sqlQuery = "SELECT top 5 AnalysisName, SUM(TblAnalysis.Id) AS TOTAL FROM TblAnalysis,TblInvoContent where TblAnalysis.Id=TblInvoContent.AnalysisId GROUP BY AnalysisName ORDER BY TOTAL DESC";
                    ConClass.da = new SqlDataAdapter(SQLConClass.sqlQuery, ConClass.con);
                    ConClass.da.Fill(dTChart);
                }
                catch (Exception ex)
                ...
                chrtTopFive.Series["Item"].XValueMember = dTChart.Columns["AnalysisName"].ToString();
...

            {
                ...
                {
                    dTChart = new DataTable();
                    SQLConClass.sqlQuery = "SELECT top 5 AnalysisName, SUM(TblAnalysis.Id) AS TOTAL FROM TblAnalysis,TblInvoContent where TblAnalysis.Id=TblInvoContent.AnalysisId GROUP BY AnalysisName ORDER BY TOTAL ASC";
                    ConClass.da = new SqlDataAdapter(SQLConClass.sqlQuery, ConClass.con);
                    ConClass.da.Fill(dTChart);
                }
                catch (Exception ex)
                ...
                foreach (DataRow itm in dTChart.Rows)
                    chrtlowestFive.Series["Item"].Points.AddXY(itm["AnalysisName"], itm["Total"]);
            }


            // **********************--- مبيعات الأصناف خلال شهر معين**********************---------
            dTChart = new DataTable();
            SQLConClass.sqlQuery = "SELECT AnalysisName,SUM(TblAnalysis.Id)AS TOTAL FROM TblAnalysis,TblInvoContent,TblInvo where TblAnalysis.ID=TblInvoContent.AnalysisId And TblInvo.ID=TblInvoContent.InvoID And TblAnalysis.DEL='FALSE' And YEAR([Date]) =" + cmbYear.Text + " And MONTH([Date]) =" + cmbMonth.Text + " GROUP BY  AnalysisName ORDER BY TOTAL DESC";
            ...
            {
                string SeriesName = Row["AnalysisName"].ToString();
        ...
        private void btnAddToChart_Click(object sender, EventArgs e)
        {
            ...
            dTChart = new DataTable();
            SQLConClass.sqlQuery = "SELECT AnalysisName,SUM(TblAnalysis.Id)AS TOTAL,MONTH([Date]) AS [month] FROM TblAnalysis,TblInvoContent,TblInvo where TblAnalysis.ID=TblInvoContent.AnalysisId And TblInvo.ID=TblInvoContent.InvoID And TblAnalysis.ID=" + cmbItemChart.SelectedValue + " And YEAR([Date]) =" + cmbYear1.Text + " GROUP BY  AnalysisName,MONTH([Date]) ORDER BY [month] ASC";
